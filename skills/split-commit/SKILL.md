...
Step 5 的骨架输出到终端后，询问用户：

```
模块骨架已输出。你可以对以下内容提出修改建议:

  - 模块依赖关系是否有误？
  - 文件耦合聚类是否合理？是否有需要合并或拆分的文件组？
  - 文件组的实现顺序是否需要调整？
  - 耦合度标注为"不确定"的文件组是否需要人工判断？

如无修改建议，回复"ok"将骨架写入文档。
>
```

**用户提供修改建议**：根据反馈调整骨架，重新输出关键变更部分（不全量输出），再次确认。

**用户确认后**：将最终模块骨架写入 `draft/skeletons/<commit-hash>.md`。文件内容为完整的 Step 5 输出格式。写入后告知：

```
骨架文档已写入: docs/skeletons/<commit-hash>.md
后续可随时查阅，避免在终端翻看长输出。
```

**若骨架涉及的文件组 ≤ 2 且总行数 ≤ 100**：直接应用修改建议（如有），不询问是否写入文档——屏幕阅读即可。

### Step 7: 若 commit 无需拆分
