# Trusted Commit Planner (TCP)

> 基于 Claude Code 的软件开发方法论工具包，为 AI 辅助编译器开发强制执行工业级的提交规范和测试纪律。

TCP将 LLVM、GCC、Triton、IREE 等成熟项目的贡献规范转化为一套可执行的 Artifact——Skill、Hook、Slash Command——将 AI 的代码生成行为约束到手工程序员的工程水准。目标用户是用 AI 工具探索编译器代码，但拒绝让 AI 侵蚀自身代码理解力的研究人员和研究生。
...
```
trusted-commit-planner/
├── CLAUDE.md.template                # 项目宪法模板，由setup skill处理化
├── README.md                         # 本文档（中文入口）
├── .claude/
